...
local debounceWait -- simple timer
local function scheduleSearch()
    if debounceWait then debounceWait:Disconnect() end
    local ev; ev = RunService.Heartbeat:Connect(function(dt)
        if not debounceWait then return end
    end)
end
